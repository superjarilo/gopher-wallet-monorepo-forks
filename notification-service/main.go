...
func main() {
	...
	// 1. Инициализируем быстрый кэш Redis 8.0
	...
	// 2. Инициализируем Kafka Consumer (Reader)
	...
	// 3. Настраиваем Graceful Shutdown для воркера
	...
	// 4. Основной бесконечный цикл обработки событий
	for {
		...
		// Имитируем отправку Push-уведомления пользователю в консоль
		fmt.Printf("\n[PUSH NOTIFICATION SENT] Юзер '%s': Операция %s на сумму %.2f руб. успешно проведена!\n", 
			event.UserID, event.Type, float64(event.Amount)/100.0)

		// 5. Записываем транзакцию в Redis (Sorted Set - ZSET) по ключу юзера
